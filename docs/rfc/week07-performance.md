...
- 현재: Todo 7 semantic shell candidate를 유지하기로 결정하고 source·측정·시각 근거를 기록했다.
- 대기: Todo 8 이후 source 실험, 상품 여섯 상태, metadata, Basic After와 최종 회귀 검증이다.
...
- 상품 목록 여섯 시나리오
- Todo 8 displayed image candidate audit와 이후 적용 가능한 Hero 실험

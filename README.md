...
> Next.js(App Router) + React 19 + TypeScript. 1~3주차 React+Vite 산출물은 개인 브랜치 히스토리와 아래 기록에 남아 있습니다.
...
src/
  ...
  views/                   # Next 예약어 pages 대신 쓰는 화면 조립 레이어
  widgets/
  features/
  entities/
  shared/
...
> `src/app` 라우트 파일은 프레임워크 entry point로 두고, 실제 상품 목록 화면과 상태 조립은 FSD 레이어(`views`, `widgets`, `features`, `entities`, `shared`)에 둡니다.
...
| ----------------------------------------------- | ----------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------ |
| `src/views/product-list/ui/ProductListPage.tsx` | 페이지 조립             | widgets, features, hooks 조합                           | 화면 블록과 handler 연결만 남아 있어 view 조립 역할로 충분하다.          |

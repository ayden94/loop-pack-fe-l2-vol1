...
): Promise<HomeResponse | ApiErrorResponse> =>
  (await response.json()) as HomeResponse | ApiErrorResponse

const asHome = (body: HomeResponse | ApiErrorResponse): HomeResponse => {
  if ('banner' in body) {
    return body
  }
  throw new Error('expected HomeResponse')
}
...
describe('GET /api/home', () => {
  it('returns banner, categories, popular products, and new products', async () => {
    ...
    expect(body.popularProducts.map((product) => product.id)).toEqual([
      'p21',
      'p11',
      'p15',
      'p8',
    ...
    expect(body.newProducts.map((product) => product.id)).toEqual([
      'p26',
      'p6',
      'p27',
      'p24',
